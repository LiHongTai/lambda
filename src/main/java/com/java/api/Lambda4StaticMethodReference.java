...
public class Lambda4StaticMethodReference {
	...
	public static void main(String[] args) {
		//沒有输入，只有输出
		Supplier<String> string4Supplier = () -> Lambda4StaticMethodReference.get();
		System.out.println("复杂Lambda表达式：" + string4Supplier.get());

		// 静态方法的引用实现
		Supplier<String> stringSupplier4StaticMehtodReference = Lambda4StaticMethodReference::get;
		System.out.println("通过方法的引用来实现Lambda表达式:" + stringSupplier4StaticMehtodReference.get());
		
		//只有输入，没有输出
		Consumer<String> string4Consumer = string -> Lambda4StaticMethodReference.set(string);
		string4Consumer.accept("一般方式Lambda表达式实现");
		
		Consumer<String> stringConsumer4StaticMehtodReference = Lambda4StaticMethodReference::set;
